...
function startGame(user_name_of_game) {
    let myName = user_name_of_game
        ...
        update() {
            ...
            if (this.life>0) {
                ...
                if (this.heroAnimation.picNum=='013') {
                    cancelAnimationFrame(gameId)
                    c.fillStyle = 'red'
                    c.fillRect(canvas.width/2-400,canvas.height/2-75,300,150)
                    c.fillStyle = 'black'
                    c.fillText('Your Score: '+score,canvas.width/2-400,canvas.height/2-75+40)
                    c.fillStyle = 'red'
                    c.fillRect(canvas.width/2+100,canvas.height/2-200,300,300)
                    c.fillStyle = 'black'
                    c.fillText('Leaderboard',canvas.width/2+100,canvas.height/2-200+40)
                    c.font = "20px serif";
                    ...
                    for (let i = 0; i < 10; i++) {
                        if (i<=leader.length) {
                            c.fillText(`${i+1}. ${leader[leader.length-1-i].Name}   ${leader[leader.length-1-i].score}`,canvas.width/2+100,canvas.height/2-200+80+20*i)
                        }else{
                            break;
                        }
                    }
